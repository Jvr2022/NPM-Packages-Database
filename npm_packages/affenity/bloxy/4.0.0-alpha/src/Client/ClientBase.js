...
module.exports = class ClientBase extends EventEmitter {
	constructor (options = {}) {
		...
		this._setupMethods();
		if (!this.options.jar) this.options.jar = this.request.module.jar();
	}

	_setupMethods () {
		const clientMethodsStore = require("./methods")({ client: this });
		...
		for (let [key, value] of Object.entries(clientMethodsRegister)) {
			if (!this[key]) {
				...
				console.warn(this.errors.methodRegister.alreadyAssigned(key));
			}
		}
	}
	...
	setupMethod (filePath, name) {
		...
		const tmpStore = this.src.prepareFunctions.byConfigs({ client: this }, file.conf);
	...
	extend (extendedPath, under) {
		...
		const preparedMethods = this.src.prepareFunctions.byPath({ client: this }, extendedPath);
