...
module.exports = (client, message) => {
	...
	if (messageType === "FriendshipDestroyed") {
		if (client.user.id === args.UserId1) return client.emit("friendLost", args.UserId2);
	} else if (messageType === "FriendshipRequested") {
		return client.emit("friendRequest", args.UserId1);
	} else if (messageType === "FriendshipCreated") {
		return client.emit("friendAdded", args.UserId1);
	}
};
