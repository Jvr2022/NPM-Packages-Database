...
class ClientFunctions extends EventEmitter {
    ...
    async getFollowers (opts={}) {
        if (this._client.account && this._client.account.userId !== undefined && opts.userId === undefined) opts = Object.assign(opts, {userId: this._client.account.userId});
    ...
    async ownsGamepass (userId, passId) {
        return Methods.ownsGamepass({ userId: userId, passId: passId}, this._client);
    }
    ...
    setProxy (url) {
        this._client._setup.authTab.ProxyUrl = url;
    }

    async signup (setup) {
        return Methods.signup(setup || {}, this._client)
    }
