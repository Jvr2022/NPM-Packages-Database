...
export class RouterResponse<AdditionalDataType extends any> {
    ...
    buildResponse (): BuiltResponse<AdditionalDataType> {
        ...
        return {
            tasks: this.response.tasks,
            response: this.response.customResponse ? this.response.customResponse : this.response.type !== "redirect" ? new Response(
                this.response.body,
                this.transformOptions()
            ) : Response.redirect(this.response.redirect!.url, this.response.statusCode),
            routerResponse: this,
            routerRequest: this.request
        };
    }
